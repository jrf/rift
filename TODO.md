...
- [ ] Harden terminal state and interactive output — parse split OSC 7 sequences incrementally, sanitize replayed OSC title/cwd metadata, separate grid/mode/metadata/cursor serialization, share cell traversal across exporters, and preserve complete output frames under backpressure; final client-output and terminal-control drains are now bounded so detach cannot hang on stalled stdout #refactor
- [ ] Make CLI parsing and command transport composable — return `Result<Command, CliError>` instead of exiting during parsing, validate unknown/excess arguments consistently, centralize command aliases/help/completion metadata and session operand resolution, and deduplicate `send`/`print`; `write` now uses acknowledged `RunQuiet` chunks instead of timing-based pacing #refactor
